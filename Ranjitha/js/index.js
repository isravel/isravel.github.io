...
var Movie = function () {
	...
	_createClass(Movie, [{
		key: '_startEyes',
		value: function _startEyes() {
			var _this = this;

			// start eye blinks...
			this.tweens.gopherEyeBlinks = TweenMax.to(this.actors.gopherEyeBlinks, 0.6, { repeat: -1, autoAlpha: 0, repeatDelay: 3, ease: new SteppedEase(1) });
			...
			stage.mousemove(function (e) {
				var radius = 28,
				    ...
				    mx = e.screenX,
				    my = e.screenY,
				    gopherX = _this.actors.gopher.getBBox().x + _this.actors.gopher.getCTM().e,
				    gopherY = _this.actors.gopher.getBBox().y + _this.actors.gopherRightEye.getBBox().height + _this.actors.gopher.getCTM().f,
				    flip = mx < gopherX ? -1 : 1,
				    deg = Math.atan((my - gopherY) / (mx - gopherX)),
				    dy = radius * Math.sin(deg) * flip,
				    dx = radius * Math.cos(deg) * flip + xoff;

				TweenMax.set(_this.actors.gopherPupils, { x: dx, y: dy, transformOrigin: 'center center' });
			});
		}
	}, {
		key: '_redrawString',
		value: function _redrawString() {
		...
		key: '_balloonDrag',
		value: function _balloonDrag(me) {
			...
			var tween = Draggable.create(me.actors.balloon, {
				...
				onDrag: function onDrag(e) {
					...
					var confetti = [].concat(_toConsumableArray(me.actors.balloonConfettiLeft), _toConsumableArray(me.actors.balloonConfettiRight));
		...
		value: function setStage() {
			...
			clearTl.set(actors.greeting, { autoAlpha: 0 }).set([actors.hatConfettiLeft, actors.hatConfettiRight], { autoAlpha: 0 }).set([actors.balloonConfettiLeft, actors.balloonConfettiRight], { autoAlpha: 0 }).set(actors.partyHat, { autoAlpha: 0, transformOrigin: '50% 50%' }).set(actors.balloon, { autoAlpha: 0, x: 0, y: 0, transformOrigin: 'bottom center' }).set(actors.balloonString, { autoAlpha: 0.6 });
		...
		key: 'enterPartyHat',
		value: function enterPartyHat() {
			console.log('enter party hat...');
			var actors = this.actors;
			var hatTl = new TimelineMax({ delay: 1.5 });

			hatTl.fromTo(actors.partyHat, 0.6, { scale: 0.2 }, { scale: 1, autoAlpha: 1, ease: Elastic.easeInOut }).add('hat').staggerFromTo(actors.hatConfettiLeft, 1, { x: '+=50', scale: 0.05 }, { x: 0, scale: 1, autoAlpha: 1, ease: Power4.easeOut }, 0.01, 'hat-=0.4').staggerFromTo(actors.hatConfettiRight, 1, { x: '-=50', scale: 0.05 }, { x: 0, scale: 1, autoAlpha: 1, ease: Power4.easeOut }, 0.01, 'hat-=0.4').add('confetti').staggerTo(actors.hatConfettiLeft, 10, { x: '-=50', autoAlpha: 0 }, 0.01, 'confetti-=0.6').staggerTo(actors.hatConfettiRight, 10, { x: '+=50', autoAlpha: 0 }, 0.01, 'confetti-=0.6');

			return hatTl;
		}
	}, {
		key: 'enterBalloon',
		value: function enterBalloon() {
			...
			balloonTl.to(actors.balloonString, 0.4, { strokeDashoffset: 0, ease: Linear.easeNone }).fromTo(actors.balloon, 1, { scale: 0.1 }, { scale: 1, autoAlpha: 1, ease: Elastic.easeInOut }, '-=0.3').add('balloon').staggerFromTo(actors.balloonConfettiLeft, 1.4, { x: '+=50', scale: 0.05 }, { x: 0, scale: 1, autoAlpha: 0.9, ease: Power4.easeOut }, 0.01, 'balloon-=0.5').staggerFromTo(actors.balloonConfettiRight, 1.4, { x: '-=50', scale: 0.05 }, { x: 0, scale: 1, autoAlpha: 0.9, ease: Power4.easeOut }, 0.01, 'balloon-=0.5').add('confetti').staggerTo(actors.balloonConfettiLeft, 10, { x: '-=20', autoAlpha: 0.7 }, 0.01, 'confetti-=0.6').staggerTo(actors.balloonConfettiRight, 10, { x: '+=20', autoAlpha: 0.7 }, 0.01, 'confetti-=0.6');

			return balloonTl;
		}
	}, {
		key: 'enterGreeting',
		value: function enterGreeting() {
			console.log('enter greeting ...');
			var actors = this.actors;
			var greetTl = new TimelineMax();

			greetTl.to(actors.greeting, 1, { autoAlpha: 1, onComplete: this._balloonDrag, onCompleteParams: [this] });
		...
		value: function go() {
			...
			masterTl.add(this.setStage(), 'scene-clear').add(this.enterPartyHat(), 'scene-hat').add(this.enterBalloon(), '-=10.2', 'scene-balloon').add(this.enterGreeting(), '-=10.2', 'scene-greeting');
		}
	}]);
